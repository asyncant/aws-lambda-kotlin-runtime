...
aws cloudformation deploy \
  ...
  --stack-name "kotlin-native-lambda-function-urls" \
  --capabilities CAPABILITY_IAM

url=$(aws cloudformation describe-stacks --stack "kotlin-native-lambda-function-urls" --query "Stacks[0].Outputs[?OutputKey=='url'].OutputValue" --output text)
echo "###################################################################"
